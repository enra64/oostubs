...
class O_Stream : public Stringbuffer{
  ...
  public:
     ...
     *
     * char* prints a C string, i.e. all characters until the termination character 0 is found
     * void* prints the address of the pointer in hexadecimal
     *
    ...
    **/    
    void writeIntoBuffer(unsigned long value);

    /** \brief helper function to print the current base number prefixes
    **/    
    void printBasePrefix();
};
 ...
 * called manipulators. It is even possible to embed the output of the method
 * into the input of the stream.
 * Main goal of the manipulators is to influence the display of the following 
 * output (e.g. by choosing a basis for the display of digits).
 *
 * To indicate with which base a number is printed,
 * an indicator prefix should preceed the actual number.
 *
 * Prefixes for bases:
 *
 * bin: 0b
 ...
 * hex: 0x
 *
 * The position of a minus sign is before the prefix.
 *
 */ 
